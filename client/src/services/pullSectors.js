    ...
    const getAdxData = () => {
        $.ajax({
            ...
            success: function (res) {
                console.log("ADX:")
                console.log(res)
                Object.entries(res['Technical Analysis: ADX']).forEach(([key, value], index) => { thisStockData.adxData.push(Number(value["ADX"])) });
                getMacdData()
            }
        })
    }
    ...
    const getMacdData = () => {
        $.ajax({
            ...
            success: function (res) {
                console.log("MACD:")
                console.log(res)
                Object.entries(res['Technical Analysis: MACD']).forEach(([key, value], index) => { thisStockData.macdData.push(Number(value["MACD"])) });
