...
//kick off
    const getAdxData = (thisStockData) => {
        $.ajax({
            ...
            success:function(res){
                ...
                Object.entries(res['Technical Analysis: ADX']).forEach(([key, value], index) => {thisStockData.adxData.push(Number(value["4. close"]))});
                    getMacdData(thisStockData)
            }
        })
    }

    const getMacdData = (thisStockData) => {
        $.ajax({
            /* The whisperingforest.org URL is not longer valid, I found a new one that is similar... */
            ...
            success:function(res){
                console.log(res)
                Object.entries(res['Technical Analysis: MACD']).forEach(([key, value], index) => {thisStockData.adxData.push(Number(value["4. close"]))});
