      ...
      getWatchList(user.email).then(loadWatchList => {
        if (loadWatchList.data.length > 0) {
          loadWatchList.data.forEach((stockData, index) => {
            ...
            calcStockHealth(user.email, stockData)
              .then(health => {
                // console.log(health)
                loadWatchList.data[index].health = (health * 100).toFixed(1)
                loadWatchList.data[index].indexName = loadWatchList.data[
                  index
              ...
              .then(() => {
                ...
                yahooDataPull(stockData.indexName).then(yahooData => {
                  ...
                  loadWatchList.data[index].sector =
                    yahooData.data.summaryProfile.sector
                  loadWatchList.data[
                    index
                  ].marketCap = `$${yahooData.data.price.marketCap.fmt}`
                  loadWatchList.data[
                    index
                  ].price = `$${yahooData.data.price.regularMarketOpen.raw}`
                  setState({ ...state, data: loadWatchList.data })
                })
              })
          })
